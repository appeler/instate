...
class InRollsLnData:
    ...
    @staticmethod
    def load_data(file_name: str) -> str | os.PathLike:
    ...
    @staticmethod
    def load_instate_data(dataset: str) -> str | os.PathLike:
    ...
    @staticmethod
    def load_instate_model(model: str = "gru") -> str | os.PathLike:
    ...
    @classmethod
    def pred_last_state(
        cls, df: pd.DataFrame, lastnamecol: str, k: int = 3
    ) -> pd.DataFrame:
        """
    ...
    @classmethod
    def last_state(
        cls, df: pd.DataFrame, lastnamecol: str, dataset: str = "v1"
    ) -> pd.DataFrame:
    ...
    def infer(lastname, char2idx, idx2lang, model, device):
        with torch.no_grad():
            ...
            last_name_tensor = (
                torch.tensor(last_name_indices, dtype=torch.long)
                .unsqueeze(0)
                .to(device)
            )
            ...
            if (
                pred_third_lang == pred_first_lang
                or pred_third_lang == pred_second_lang
            ):
                pred_third_lang = torch.topk(out3, k=3, dim=1)[1][0][1]
            return [
                idx2lang[pred_first_lang.item()],
                idx2lang[pred_second_lang.item()],
                idx2lang[pred_third_lang.item()],
            ]
    ...
    def predict_lang(df: pd.DataFrame, lastnamecol: str) -> pd.DataFrame:
        ...
        # idx2char = {idx: char for char, idx in char2idx.items()}  # unused
        ...
        for lastname in df[lastnamecol]:
            pred_langs.append(
                InRollsLnData.infer(lastname, char2idx, idx2lang, model, device)
            )
