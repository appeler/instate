...
class ContourPen(BasePen):
    ...
    def __init__(self, glyph_set, scale):
        """Initialize a contour collector at the requested scale."""
    ...
    def _curveToOne(self, first, second, last):  # noqa: N802
    ...
    def raster(self):
        """Render the collected paths on a fixed comparison canvas."""
...
def measure(glyph, glyph_set, scale):
    """Return scaled glyph bounds, preserving an empty glyph as ``None``."""
...
def close_metrics(width, bounds, reference):
    """Return whether advance width and bounds match within tolerance."""
...
class ReferenceFonts:
    ...
    def __init__(self, paths):
        """Load printable ASCII reference outlines from the supplied fonts."""
    ...
    def decode(self, charstring, matrix, glyph_name):
        """Decode one embedded CFF glyph after validating its font matrix."""
    ...
    def decode_truetype(self, font, gid):
        """Decode one embedded TrueType glyph by its physical outline."""
    ...
    def match(self, glyph, glyph_set, scale):
        """Match a glyph to exactly one printable reference character."""
...
class DocumentFonts:
    ...
    def __init__(self, document, references):
        """Bind a PDF document to the validated reference font matcher."""
        self.document, self.references = document, references
        self.fonts = {}

    def page(self, page):
        """Build an unambiguous glyph map for one PDF page."""
...
def extract_page(page, fonts):
    """Decode every physical glyph group on one page."""
