...
def join_lines(lines, field="candidate"):
    """Join parsed words into source lines using the requested field."""
    return "\n".join(" ".join(word[field] for word in line) for line in lines)


def accept_name(candidate):
    """Accept only complete printable ASCII name candidates."""
    if not candidate:
...
def source_name_labels(glyphs):
    """Count decoded elector-name labels outside bold header text."""
...
def parse_card(box, glyphs):
    """Parse one English elector card and retain rendering evidence."""
...
def component_type(text):
    """Return the supplement event type encoded in a component heading."""
...
def npr_heading(text):
    """Return whether a heading marks records ineligible for assembly voting."""
    return bool(
        re.search(
...
def closing_summary(page, lines):
    """Parse a closing summary and retain its numeric source rows."""
...
def parse_pdf(path, references):
    """Parse and reconcile one English electoral-roll PDF."""
    import pymupdf
...
def initialize_worker(paths):
    """Initialize process-local reference fonts for parallel parsing."""
    global _REFERENCES
    _REFERENCES = ReferenceFonts(paths)


def parse_job(path):
    """Parse one worker job after reference-font initialization."""
...
def main():
    """Parse the configured English roll split into audited Parquet files."""
